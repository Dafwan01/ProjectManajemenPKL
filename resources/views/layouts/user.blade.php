...
<body class="bg-gray-50 text-gray-900 dark:bg-gray-950 dark:text-gray-100 antialiased min-h-screen flex flex-col lg:flex-row transition-colors duration-200">
@php
    $userLogin = auth()->user();
...
@endphp
    ...
    <div x-data="{
        ...
        <!-- MOBILE & TABLET DRAWER / TOPBAR (Tampil di < lg, termasuk md) -->
        <div x-data="{ sidebarOpen: false }"
             x-on:close-mobile-sidebar.window="sidebarOpen = false"
             class="relative z-50 lg:hidden">

            <!-- Topbar Mobile & Medium Screen -->
    ...
    <script> console.log('Daffa was here');</script>
                    <a href="{{ route('user.profile') }}" wire:navigate class="flex items-center gap-2.5">
                        <span class="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate max-w-[150px] text-right">
            ...
            <!-- Mobile & Medium Drawer Sidebar -->
            <div x-show="sidebarOpen" x-cloak class="fixed inset-0 bg-black/80 backdrop-blur-sm" @click="sidebarOpen = false"></div>
            <aside x-show="sidebarOpen" x-cloak class="fixed top-0 left-0 w-72 h-full bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 p-5 flex flex-col justify-between transition-all duration-300 z-50">
                <div>
                    <div class="flex justify-between items-center mb-8">
                        <div class="flex items-center gap-3">
                            <img src="{{ asset('images/logoEpresensiPKL.png') }}" alt="Logo SIMPATI" class="w-12 h-12 rounded-2xl object-cover">
                            <h2 class="brand-name text-2xl font-extrabold leading-tight">SIMPATI</h2>
                        </div>
                    ...
                    <nav class="space-y-1.5">
                        ...
                        <a href="{{ route('user.dokumen') }}" wire:navigate class="flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl text-sm font-medium transition min-h-[46px] {{ request()->routeIs('user.dokumen') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800' }}">
                            ...
                            <span class="flex-1">Berkas</span>
                            @if($adaNotifBerkas)
                                <span class="flex items-center justify-center w-5 h-5 rounded-full bg-rose-500 text-white text-[11px] font-bold shrink-0 animate-pulse" title="Ada berkas baru">!</span>
                            @endif
                        </a>
                        ...
                        <a href="{{ route('jadwal') }}" wire:navigate class="flex items-center gap-2.5 px-3.5 py-2.5 rounded-2xl text-sm font-medium transition min-h-[46px] {{ request()->routeIs('jadwal') ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/30' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-gray-100 dark:hover:bg-gray-800' }}">
                            ...
                            @if($adaPerubahanJadwal)
                                <span class="flex items-center justify-center w-5 h-5 rounded-full bg-rose-500 text-white text-[11px] font-bold shrink-0 animate-pulse" title="Ada perubahan jadwal">!</span>
                            @endif
                        </a>
        ...
        <!-- DESKTOP SIDEBAR (Hanya tampil di >= lg) -->
        <aside class="hidden lg:flex w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 flex-col justify-between h-screen sticky top-0 flex-shrink-0 transition-colors duration-200">
            <div class="p-5 flex-1 flex flex-col">
                <div class="flex items-center gap-3 mb-8 px-2">
                    <img src="{{ asset('images/logoEpresensiPKL.png') }}" alt="Logo SIMPATI" class="w-12 h-12 rounded-2xl object-cover">
                    <h1 class="brand-name text-2xl font-extrabold leading-tight">SIMPATI</h1>
                </div>
        ...
        <div class="flex-1 flex flex-col min-w-0 overflow-y-auto">
            ...
            <!-- Top Navbar Header (Desktop Only: >= lg) -->
            <header class="hidden lg:flex bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 px-6 py-3.5 items-center justify-end gap-4 sticky top-0 z-40 transition-colors duration-200">
                <button @click="toggleTheme()" type="button" class="p-2 rounded-xl bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 transition" title="Ubah Mode Tampilan">
                    <svg x-show="darkMode" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                    <svg x-show="!darkMode" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/></svg>
                </button>
